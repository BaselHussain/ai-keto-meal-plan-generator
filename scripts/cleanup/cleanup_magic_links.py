#!/usr/bin/env python3
"""Cleanup job: Delete expired magic link tokens (T132).

Retention policy: magic_link_tokens where expires_at < NOW().
Schedule: Daily
"""
import argparse
import asyncio
...
SCRIPT_NAME = "cleanup_magic_links"
...
async def cleanup(dry_run=False):
    ...
    async with sf() as session:
        total = (await session.execute(
            select(func.count()).where(MagicLinkToken.expires_at < now)
        )).scalar_one()
        ...
        async with sf() as session:
            try:
                result = await session.execute(
                    select(MagicLinkToken.id).where(MagicLinkToken.expires_at < now).limit(BATCH_SIZE)
                )
                ids = [r[0] for r in result.fetchall()]
                if not ids:
                    break
                await session.execute(delete(MagicLinkToken).where(MagicLinkToken.id.in_(ids)))
...
def main():
    parser = argparse.ArgumentParser(description="Delete expired magic link tokens (T132)")
