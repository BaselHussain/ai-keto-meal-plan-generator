#!/usr/bin/env python3
"""Cleanup job: Delete meal plans older than 90 days (T130).

Retention policy: meal_plans where created_at < NOW() - 90 days.
Schedule: Daily
"""
import argparse
import asyncio
...
SCRIPT_NAME = "cleanup_meal_plans"
...
async def cleanup(dry_run=False):
    ...
    async with sf() as session:
        total = (await session.execute(
            select(func.count()).where(MealPlan.created_at < cutoff)
        )).scalar_one()
        ...
        async with sf() as session:
            try:
                result = await session.execute(
                    select(MealPlan.id).where(MealPlan.created_at < cutoff).limit(BATCH_SIZE)
                )
                ids = [r[0] for r in result.fetchall()]
                if not ids:
                    break
                await session.execute(delete(MealPlan).where(MealPlan.id.in_(ids)))
...
def main():
    parser = argparse.ArgumentParser(description="Delete meal plans older than 90 days (T130)")
