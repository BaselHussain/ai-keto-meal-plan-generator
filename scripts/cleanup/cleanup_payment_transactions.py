#!/usr/bin/env python3
"""Cleanup job: Delete payment transactions older than 1 year (T133A).

Retention policy: payment_transactions where created_at < NOW() - 365 days.
Schedule: Monthly
"""
import argparse
import asyncio
...
SCRIPT_NAME = "cleanup_payment_transactions"
...
async def cleanup(dry_run=False):
    ...
    async with sf() as session:
        total = (await session.execute(
            select(func.count()).where(PaymentTransaction.created_at < cutoff)
        )).scalar_one()
    ...
    while True:
        async with sf() as session:
            try:
                ...
                if not ids:
                    break
                await session.execute(delete(PaymentTransaction).where(PaymentTransaction.id.in_(ids)))
...
def main():
    parser = argparse.ArgumentParser(description="Delete payment transactions older than 1 year (T133A)")
