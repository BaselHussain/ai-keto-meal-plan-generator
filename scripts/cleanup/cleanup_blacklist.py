#!/usr/bin/env python3
"""Cleanup job: Delete expired email blacklist entries (T133).

Retention policy: email_blacklist where expires_at < NOW().
Schedule: Daily
"""
import argparse
import asyncio
...
async def cleanup(dry_run=False):
    ...
    logger.info(f"Starting (dry_run={dry_run}).")

    sf = _build_session_factory()

    async with sf() as session:
        total = (await session.execute(
            select(func.count()).where(EmailBlacklist.expires_at < now)
        )).scalar_one()
        ...
        async with sf() as session:
            try:
                result = await session.execute(
                    select(EmailBlacklist.id).where(EmailBlacklist.expires_at < now).limit(BATCH_SIZE)
                )
                ids = [r[0] for r in result.fetchall()]
                if not ids:
                    break
                await session.execute(delete(EmailBlacklist).where(EmailBlacklist.id.in_(ids)))
...
def main():
    parser = argparse.ArgumentParser(description="Delete expired email blacklist entries (T133)")
