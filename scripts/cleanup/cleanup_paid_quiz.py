#!/usr/bin/env python3
"""Cleanup job: Delete paid quiz responses 24 hours after PDF delivery (T128).

Retention policy: quiz_responses where pdf_delivered_at < NOW() - 24 hours.
    ...
    async with sf() as session:
        total = (await session.execute(
            select(func.count()).where(and_(
                QuizResponse.pdf_delivered_at.isnot(None),
                QuizResponse.pdf_delivered_at < cutoff
            ))
        )).scalar_one()
...
def main():
    parser = argparse.ArgumentParser(description="Delete paid quiz responses 24h after delivery (T128)")
