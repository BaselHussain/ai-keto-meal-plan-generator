...
def _make_context_manager_mock(session: AsyncMock) -> MagicMock:
    """
    Build a MagicMock that behaves like an async context manager yielding
    the provided session.  Used to patch ``get_db_context``.
    """
    cm = MagicMock()
...
async def test_refund_webhook_integration():
    ...
    # Mock session execution - first call returns payment, subsequent calls
    # return empty results for refund-count and blacklist queries.
    payment_result = MagicMock()
    payment_result.scalar_one_or_none.return_value = mock_payment

    refund_count_result = MagicMock()
    refund_count_result.scalars.return_value.all.return_value = []

    mock_session.execute.side_effect = [payment_result, refund_count_result]

    with patch('src.services.refund_service.get_db_context', return_value=_make_context_manager_mock(mock_session)):
...
async def test_process_refund_with_no_related_meal_plan():
    ...
    # payment.meal_plan is None - SQLAlchemy will return None for an uninitialised
    # relationship attribute on a detached instance.
